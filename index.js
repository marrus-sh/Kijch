...
export const {
	...
	let SuperCharacter = class Character { constructor ( ) { if (!(this instanceof Character)) throw new TypeError("Invalid constructor.") } }
	Character = class extends SuperCharacter { constructor ( context, props ) {
		...
		Object.defineProperties(this, {
			...
			, glyphs: { enumerable: true, get: () => {
				const result = { }
				props.glyphs.forEach(( glyph ) => {
					const glyphObject = new Glyph(glyph)
					Object.defineProperty(result, `${ glyphObject.width }x${ glyphObject.height }`, { enumerable: true, get: ( ) => glyphObject }) })
				return result } }
	...
	return {
		Character: SuperCharacter // Works with `typeof` but cannot construct
				...
				for ( ; index < lines.length ; line = lines[++index] ) {
					...
					if ( /^ *U\+(?:0*(?:10|[1-9A-F])[0-9A-F]{0,4}|0+)(?: [^\x00-\x1F\x7F-\x9F\uD800-\uDFFF\uFDD0-\uFDEF\uFFF0-\uFFFF\u{1FFFE}\u{1FFFF}\u{2FFFE}\u{2FFFF}\u{3FFFE}\u{3FFFF}\u{4FFFE}\u{4FFFF}\u{5FFFE}\u{5FFFF}\u{6FFFE}\u{6FFFF}\u{7FFFE}\u{7FFFF}\u{8FFFE}\u{8FFFF}\u{9FFFE}\u{9FFFF}\u{AFFFE}\u{AFFFF}\u{BFFFE}\u{BFFFF}\u{CFFFE}\u{CFFFF}\u{DFFFE}-\u{E0FFF}\u{EFFFE}\u{EFFFF}\u{FFFFE}\u{FFFFF}\u{10FFFE}\u{10FFFF}]*)?$/u.test(line) ) { // <CharacterDefinition>
						...
						while ( match = line.match(/^ *# *([0-9A-F]{8,}) *$/u) ) { // <Glyphs>
							let hex = match[1],
								sqrt = 0
							while ( !((sqrt = Math.floor(Math.sqrt(hex.length * 4))) * sqrt == hex.length * 4 || (sqrt = Math.floor(Math.sqrt(hex.length * 8))) * sqrt == hex.length * 8) ) hex += "0"
